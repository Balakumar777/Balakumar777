<cfsilent>
<cfif isDefined("url.log")><cffile action="append" file="C:\inetpub\temp\log.txt" output="#Chr(13)##Chr(10)##dateTimeFormat(now(),"yyyy-mm-dd HH:mm:ss")# start model glue"></cfif>
...
<cfif isDefined("url.log")><cffile action="append" file="C:\inetpub\temp\log.txt" output="#dateTimeFormat(now(),"yyyy-mm-dd HH:mm:ss")# model glue part check point 1"></cfif>

<cfif not request._modelglue.bootstrap.blockEvent>
	<cfset ec = mg.handleRequest() />
</cfif>
<cfif isDefined("url.log")><cffile action="append" file="C:\inetpub\temp\log.txt" output="#dateTimeFormat(now(),"yyyy-mm-dd HH:mm:ss")# model glue part check point 2"></cfif>
...
<cfif isDefined("url.log")><cffile action="append" file="C:\inetpub\temp\log.txt" output="#dateTimeFormat(now(),"yyyy-mm-dd HH:mm:ss")# end model glue"></cfif>