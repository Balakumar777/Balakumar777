...
<cffunction name="init" access="public" returnType="any" output="false" hint="I build a new EventContext.">
	...
	<cfif structKeyExists(arguments, "modelglue")>
		...
		<cfset variables._urlManager = variables._modelGlue.getInternalBean("modelglue.urlManager") />
	<cfelse>
		<!--- External maps of listeners and handlers --->
	...
	<cftry>
		<cfif isArray(variables._requestPhases) and arrayLen(variables._requestPhases)>
			<cfloop from="1" to="#arrayLen(variables._requestPhases)#" index="i">
				...
				<cfif isDefined("url.log")><cffile action="append" file="C:\inetpub\temp\log.txt" output="#dateTimeFormat(now(),"yyyy-mm-dd HH:mm:ss")# EventContext.cfc part check point 1 - #i#"></cfif>
				<cfset variables._requestPhases[i].execute(this) />
				<cfif isDefined("url.log")><cffile action="append" file="C:\inetpub\temp\log.txt" output="#dateTimeFormat(now(),"yyyy-mm-dd HH:mm:ss")# EventContext.cfc part check point 2 - #i#"></cfif>
...
<cffunction name="executeEventQueue" access="public" output="false" hint="Executes all event handlers currently in the event queue and renders queued views.">
	...
	<cfif not isSimpleValue(variables._nextView)>
	 ...
	 --->
	<cfif isDefined("url.log")><cffile action="append" file="C:\inetpub\temp\log.txt" output="#dateTimeFormat(now(),"yyyy-mm-dd HH:mm:ss")# EventContext.cfc - executeEventQueue() part check point 1"></cfif>
	...
	<cfif isDefined("url.log")><cffile action="append" file="C:\inetpub\temp\log.txt" output="#dateTimeFormat(now(),"yyyy-mm-dd HH:mm:ss")# EventContext.cfc - executeEventQueue() part check point 2"></cfif>
...
<cffunction name="formatUrlParameter" output="false" hint="Formats a key/value pair for the URL.">
	...
	<cfif structKeyExists(variables, "_urlManager")>
		<cfset var urlManager = variables._urlManager />
	<cfelse>
