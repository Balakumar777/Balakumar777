...
<cffunction name="execute" returntype="void" output="false" hint="Executes the request phase.">
	...
	<cfif isDefined("url.log")><cffile action="append" file="C:\inetpub\temp\log.txt" output="#dateTimeFormat(now(),"yyyy-mm-dd HH:mm:ss")# Invocation.cfc part check point 1"></cfif>
	<!--- Tell the context to run its queue. --->
	<cfset arguments.eventContext.executeEventQueue(true) />
	<cfif isDefined("url.log")><cffile action="append" file="C:\inetpub\temp\log.txt" output="#dateTimeFormat(now(),"yyyy-mm-dd HH:mm:ss")# Invocation.cfc part check point 2"></cfif>
	<!--- onRequestEnd --->
